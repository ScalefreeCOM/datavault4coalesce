...
{%- macro datavault4coalesce__databricks_reference_table_run(node, columns) -%}
  ...
  INSERT INTO {{ ref_no_link(node.location.name, node.name) }}
    ...
    {% if historized in ['Full', 'Latest'] -%}
      ...
      {% for satellite in ns.ref_satellites_dict -%}
        ...
        WHERE {{ date_column }} != {{ datavault4coalesce__databricks_string_to_timestamp(datavault4coalesce.config.timestamp_format, datavault4coalesce.config.end_of_all_times) }}
    ...
    WHERE `{{ date_column }}` > (
            SELECT 
                COALESCE(MAX(`{{ date_column }}`),  {{ datavault4coalesce__databricks_string_to_timestamp(datavault4coalesce.config.timestamp_format, datavault4coalesce.config.beginning_of_all_times) }})
