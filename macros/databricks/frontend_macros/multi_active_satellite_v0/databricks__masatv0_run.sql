...
{%- macro datavault4coalesce__databricks_multi_active_satellite_v0_run(node, config, sources) -%}
  ...
  deduplicated_numbered_source AS (
      ...
      WHERE `{{ datavault4coalesce.config.ldts_alias }}` > (
        ...
        OR `{{ hashkey_column }}` = {{ datavault4coalesce__databricks_unknown_key() }}
        OR `{{ hashkey_column }}` = {{ datavault4coalesce__databricks_error_key() }}
      {% endif %}  
          QUALIFY
          CASE
