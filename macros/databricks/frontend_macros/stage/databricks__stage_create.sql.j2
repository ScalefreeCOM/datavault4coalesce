    ...
    {% elif node.materializationType == 'view' %}
        ...
        {% if not config.contains_multiactive_data -%}
            ...
            {% for col in sources[0].columns %}
                ...
                {% if col.hashDetails %}
                    {{ datavault4coalesce__databricks_hash(columns=col.hashDetails.columns, is_hashdiff=col.is_hd,datatype=col.dataType, algo=datavault4coalesce.config.hash) }} AS `{{ col.name }}`
        ...
        {% else %}
            ...
            {%- endfor -%}
                    ...
                    {#  {{ datavault4coalesce__databricks_hash(columns=col.hashDetails.columns, is_hashdiff=col.is_hd,datatype=col.dataType, algo=datavault4coalesce.config.hash, multi_active_key=multi_active_key, main_hashkey_col=main_hashkey_col.hk_col) }} AS `{{ col.name }}` #}
            ...
            , ghost_records AS (SELECT
                ...
                {% for col in sources[0].columns %}
                    {%- if "hashDetails" in col.keys() %}
                        {{ datavault4coalesce__databricks_ghost_record_per_datatype(col.name, col.dataType, 'unknown', hash=true, hash_algo=col.hashDetails.algorithm) }}
                    {%- else -%}
                        {{ datavault4coalesce__databricks_ghost_record_per_datatype(col.name, col.dataType, 'unknown', hash=false) }}
                    {%- endif -%}
                {% if not loop.last %},{% endif %}
                {% endfor %}
                ...
                {% for col in sources[0].columns %}
                    {%- if "hashDetails" in col.keys() %}
                        {{ datavault4coalesce__databricks_ghost_record_per_datatype(col.name, col.dataType, 'error', hash=true, hash_algo=col.hashDetails.algorithm) }}
                    {%- else -%}
                        {{ datavault4coalesce__databricks_ghost_record_per_datatype(col.name, col.dataType, 'error', hash=false) }}
                    {%- endif -%}
                {% if not loop.last %},{% endif %}
                {% endfor %}
            )
