...
{#-- Utility macro: outputs a SQL function to run a Reference Satellite V0. --#}
{%- macro datavault4coalesce__databricks_reference_satellite_v0_run(node, columns) -%}
  ...
  WITH latest_entries_in_sat AS (
    ...
    */#}
    ...
    QUALIFY ROW_NUMBER() OVER (PARTITION BY `{{ refkey_column }}` ORDER BY `{{ datavault4coalesce.config.ldts_alias }}` DESC) = 1
  ...
  deduplicated_numbered_source AS (
      ...
      {% endfor %}
          ...
          ROW_NUMBER() OVER(PARTITION BY `{{ refkey_column }}` ORDER BY `{{ datavault4coalesce.config.ldts_alias }}`) as rn
      ...
      {%- endif %}  
          ...
          QUALIFY
          CASE
              WHEN `{{ hashdiff_column }}` = LAG(`{{ hashdiff_column }}`) OVER(PARTITION BY `{{ refkey_column }}` ORDER BY `{{ datavault4coalesce.config.ldts_alias }}` ) THEN FALSE
              ELSE TRUE
          END
    ...
    WHERE NOT EXISTS (
      ...
      AND deduplicated_numbered_source.`{{ refkey_column }}` = latest_entries_in_sat.`{{ refkey_column }}`
