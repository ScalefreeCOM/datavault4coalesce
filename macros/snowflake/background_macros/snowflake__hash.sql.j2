...
{#---------------------------------------------------------------------------------#}
{%- macro datavault4coalesce__snowflake_hash(columns, alias=none, is_hashdiff=false, datatype='STRING', algo='MD5', multi_active_key=[], main_hashkey_col='') -%}
...
{%- set hash_alg = datavault4coalesce__snowflake_hash_algorithm(hash_function=algo, hash_datatype=datatype) -%}
{%- set unknown_key = datavault4coalesce__snowflake_unknown_key(hash_function=algo, hash_datatype=datatype) -%}
{%- set error_key = datavault4coalesce__snowflake_error_key(hash_function=algo, hash_datatype=datatype) -%}

{%- set attribute_standardise = datavault4coalesce__snowflake_attribute_standardise() %}
...
{%- set standardise_prefix = datavault4coalesce__snowflake_standardise_prefix(case_sensitive=hash_input_case_sensitive, 
                                                                    hash_alg=hash_alg,
...
{%- set standardise_suffix = datavault4coalesce__snowflake_standardise_suffix(case_sensitive=hash_input_case_sensitive, 
                                                                    hash_alg=hash_alg,
                                        is_hashdiff=is_hashdiff,
                                                                    datatype=hash_dtype, 
                                                                    alias=alias, 
...
{%- for column in columns -%}
    ...
    {%- if datavault4coalesce.config.hash_passthrough_input_transformations == TRUE -%}
        {%- set column_str = datavault4coalesce__snowflake_as_constant(get_source_transform(column)) -%}
    {%- else -%}
        {%- if '.' in column.name %}
            {% set column_str = column.name -%}
        {%- else -%}
            {%- set column_str = datavault4coalesce__snowflake_as_constant(column.name) -%}
        {%- endif -%}
    {%- endif -%}
