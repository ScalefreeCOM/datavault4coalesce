...
{#-- Utility macro: outputs a SQL function to run a Satellite v0 table. --#}
{%- macro datavault4coalesce__snowflake_satellite_v0_run(node, columns) -%}
  ...
  {%- set dedup_alias = "deduplicated_numbered_source" -%}
    {% for source in sources %}
      SELECT DISTINCT
      {% for col in source.columns %}
        "{{ col.name }}"
    ...
    WHERE NOT EXISTS (
      ...
      WHERE 
        {{dedup_alias}}."{{ hashdiff_column }}" = latest_entries_in_sat."{{ hashdiff_column }}"
      AND {{dedup_alias}}."{{ hashkey_column }}" = latest_entries_in_sat."{{ hashkey_column }}"

          AND {{dedup_alias}}.rn = 1
    )
