...
{%- macro datavault4coalesce__snowflake_effectivity_satellite_run(node, columns) -%}
    ...
    source_data AS (
        ...
        AND src.{{ src_ldts }} > COALESCE((
            SELECT
                MAX({{ src_ldts }})
            FROM {{ this }}
            WHERE {{ src_ldts }} != {{ datavault4coalesce__snowflake_string_to_timestamp(timestamp_format, end_of_all_times) }}
        ), {{ datavault4coalesce__snowflake_string_to_timestamp(timestamp_format, beginning_of_all_times) }} )
        {%- endif %}
    ),
    ...
    records_to_insert AS (
        ...
        FROM {{ ns.last_cte }} di
            ...
            {%- if not source_is_single_batch %}
                ...
                AND di.{{ src_ldts }} > COALESCE((SELECT MAX({{ src_ldts }}) FROM {{ this }}), {{ datavault4coalesce__snowflake_string_to_timestamp(timestamp_format, beginning_of_all_times) }} )
            {% endif %}
