...
{%- set start_date = SELECT min("{{ sources[0].columns[0].sourceColumns[0].column.name }}") FROM "{{ storageLocations[0].database }}"."{{ storageLocations[0].schema }}"."{{ sources[0].columns[0].sourceColumns[0].node.name }}" -%}
